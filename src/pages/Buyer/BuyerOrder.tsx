...
const tabs = ["รอจ่าย","จ่ายแล้ว / รอจัดส่ง","กำลังจัดส่ง","จัดส่งสำเร็จ","ยกเลิก"] as const;
...
const BuyerOrder: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 bg-gray-100 min-h-screen">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">ประวัติคำสั่งซื้อของฉัน</h2>
        ...
        <div className="flex flex-col gap-6">
          {filteredOrders.map(order => {
            ...
            const totalPrice = order.sellers.reduce(
              (sum, seller) => sum + seller.items.reduce((a,i)=>a+i.unitPrice*i.quantity,0),
              0
            );

            return (
              <div key={order.orderId} className="bg-white rounded-lg shadow-sm p-4 flex flex-col gap-4">

                {/* Header */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-0">
                  <div className="flex flex-col">
                    {order.sellers.map(seller => (
                      <h2 key={seller.sellerId} className="font-semibold text-lg text-gray-800 mb-1">
                        ร้านค้า: {seller.sellerName}
                      </h2>
                    ))}
                    <h3 className="font-bold text-lg text-gray-900">รหัสคำสั่งซื้อ: {order.orderId}</h3>
                    <span className="text-gray-600">วันที่สั่งซื้อ: {thaiDate} เวลา: {thaiTime}</span>
                  </div>

                  <div className="flex flex-wrap items-center gap-2 mt-2 md:mt-0">
                    <span className={`px-2 py-1 rounded text-sm font-semibold ${orderStatusColors[order.statusLabel]||"bg-gray-100 text-gray-800"}`}>
                      {order.statusLabel}
                    </span>
                    ...
                    {order.statusLabel === "รอจ่าย" && (
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={() => {
                            if(window.confirm("กรุณาตรวจสอบรายการและที่อยู่ก่อนชำระเงิน\nคุณยืนยันที่จะชำระเงินหรือไม่?")) {
                        ...
                        <button
                          ...
                          onClick={async () => {
                            if(window.confirm("คุณต้องการยกเลิกคำสั่งซื้อนี้หรือไม่?")) {
                              try { await cancelOrder(order.orderId); toast.success("ยกเลิกคำสั่งซื้อเรียบร้อยแล้ว"); fetchOrders(); }
                              catch(err:any){ toast.error(err.message || "ไม่สามารถยกเลิกคำสั่งซื้อได้"); }
                            }
                          }}
                        >ยกเลิก</button>
                      </div>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if(window.confirm("คุณได้รับสินค้านี้แล้วใช่หรือไม่?")) {
                            try { await updateOrderStatus(order.orderId,"สำเร็จ"); toast.success("ยืนยันการรับสินค้าเรียบร้อยแล้ว"); fetchOrders(); }
                            catch(err:any){ toast.error(err.message || "ไม่สามารถอัปเดตสถานะได้"); }
                          }
                        }}
                    ...
                    {seller.items.map(item => (
                      <li key={item.productId} className="flex gap-4 py-4 items-center">
                        <div className="w-20 h-20 flex-shrink-0 rounded overflow-hidden border border-gray-200">
                          {item.filePath ? (
                            <img src={item.filePath.includes("dropbox.com") ? item.filePath.replace("?dl=0","?raw=1") : item.filePath} alt={item.productName} className="w-full h-full object-cover"/>
                          ) : (
                            <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-400 text-xs text-center p-2">ไม่มีรูปภาพ</div>
                          )}
                        </div>

                        <div className="flex-grow flex flex-col justify-between">
                          <p className="font-semibold text-gray-900 line-clamp-2">{item.productName}</p>
                          <p className="text-sm text-gray-500 mt-1">ประเภท: {item.productTypeLabel}</p>
                          <p className="text-sm text-gray-500 mt-1">จำนวน: {item.quantity} ชิ้น</p>
                          <p className="text-red-500 font-bold mt-2">ราคาหน่วยละ : {item.unitPrice.toLocaleString()} บาท</p>
                        </div>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                  ...
                  {order.statusLabel === "รอจ่าย" && editingOrderId === order.orderId ? (
                    <div className="flex flex-col gap-2 md:gap-4">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={async () => {
                            try { await updateOrderAddress(order.orderId,tempAddress); toast.success("อัปเดตที่อยู่เรียบร้อย"); setEditingOrderId(""); fetchOrders(); }
                            catch(err:any){ toast.error(err.response?.data?.message || "ไม่สามารถอัปเดตที่อยู่ได้"); }
                          }}
                        >บันทึก</button>
                        <button className="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 transition text-sm" onClick={()=>setEditingOrderId("")}>ยกเลิก</button>
                      </div>
                    </div>
                  ) : (
                    <p className="text-gray-600 line-clamp-2">{order.address || "ไม่ระบุที่อยู่"}</p>
                  )}

                  {order.statusLabel==="รอจ่าย" && editingOrderId!==order.orderId && (
                    <button className="mt-2 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition text-sm"
                      onClick={()=>{ setEditingOrderId(order.orderId); setTempAddress(order.address); }}
                    >แก้ไข</button>
                  )}
                </div>
